        ...
        <Box sx={{ backgroundColor: '#f0f0f0', minHeight: '100vh', py: 4, px: 2 }}>
            <Box sx={{ maxWidth: 1000, margin: '0 auto' }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3 }}>
                    <Box>
                        <Typography variant="h5" sx={{ fontWeight: 'bold', color: '#0F3D1F' }}>Votre panier</Typography>
                        <Typography sx={{ color: '#999', fontSize: '13px' }}>{items.length} produit{items.length > 1 ? 's' : ''} dans votre panier</Typography>
                    </Box>
                    <Link href="/frontend">
                        <Button startIcon={<ArrowBack />} sx={{ color: '#0F3D1F', textTransform: 'none', fontWeight: 'bold' }}>
                ...
                <Box sx={{ display: 'flex', gap: 3, alignItems: 'flex-start', flexDirection: { xs: 'column', md: 'row' } }}>
                    ...
                    {/* Liste produits */}
                    <Paper elevation={3} sx={{ flex: 1, borderRadius: '20px', p: 3 }}>
                                    ...
                                    <Box key={item.id}>
                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, p: 2 }}>
                                            <Box sx={{ width: 80, height: 80, borderRadius: '10px', backgroundColor: '#e8f0eb', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                                <Image src={item.imageUrl} alt={item.name} width={70} height={70} style={{ objectFit: 'contain' }} />
                                            </Box>
                                            ...
                                            <Typography sx={{ fontWeight: 'bold', fontSize: '15px', color: '#222', minWidth: 90, textAlign: 'right' }}>
                                                {(prix * item.quantity).toLocaleString()} Fcfa
                                            </Typography>
                                            <Box sx={{ display: 'flex', alignItems: 'center', border: '1px solid #e0e0e0', borderRadius: '8px', overflow: 'hidden', ml: 1 }}>
                                            ...
                                            <IconButton onClick={() => removeFromCart(item.id)} sx={{ color: '#ccc', '&:hover': { color: '#e53935' } }}>
                    ...
                    {/* Récapitulatif */}
                    <Paper elevation={3} sx={{ width: { xs: '100%', md: 280 }, borderRadius: '20px', p: 3, flexShrink: 0, position: 'sticky', top: 20 }}>
