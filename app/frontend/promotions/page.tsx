...
export default function PromotionsPage() {
    ...
    return (
        <Box sx={{ backgroundColor: '#f5f5f5', minHeight: '100vh' }}>
            <Box sx={{ background: 'linear-gradient(135deg, #0F3D1F 0%, #168039 100%)', color: '#fff', padding: '40px 60px', display: 'flex', alignItems: 'center', gap: '16px' }}>
                <LocalOffer sx={{ fontSize: 40 }} />
                <Box>
                    <Typography sx={{ fontSize: '28px', fontWeight: 800, lineHeight: 1.1 }}>Nos Promotions</Typography>
                    <Typography sx={{ fontSize: '14px', opacity: 0.8, mt: 0.5 }}>{products.length} produits en promotion</Typography>
                </Box>
            </Box>

            <Box sx={{ display: 'flex', maxWidth: 1400, margin: '0 auto', padding: '24px', gap: '24px' }}>

                {/* Sidebar filtres */}
                <Box sx={{ width: 220, flexShrink: 0 }}>
                    <Box sx={{ backgroundColor: '#fff', borderRadius: '16px', padding: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', position: 'sticky', top: '100px' }}>
                                ...
                                return (
                                    <Box key={p.id} onClick={() => router.push(`/frontend/produit/${p.id}`)}
                                        ...
                                        <Box sx={{ padding: '12px' }}>
                                            ...
                                            <Box sx={{ fontSize: '15px', fontWeight: 800, color: '#0F3D1F' }}>{p.pricePromo!.toLocaleString()} Fcfa</Box>
